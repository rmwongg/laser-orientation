...
-   This data set has 30 variables and 603 observations.

-   The data set is organized by student ID (the 603 observations). In
    addition, course ID, total points possible for the course, and the
    total points earned, among other variables, are provided.

-   It looks like there was a 10-item survey that students completed.
    Since students' grades and points were also collected, I'm guessing
    that researchers are interested in the relationship between
    responses on the survey items and overall grades/points earned.
...
-   Rows represent the observations, individuals, cases.
...
```{r}
d %>%
  select(student_id, total_points_possible, total_points_earned, FinalGradeCEMS)
```
...
```{r}
d %>%
  filter(FinalGradeCEMS >= 65)
```

What happens when you change the cut-off from 70 to something else? Add
a thought (or more):

-   I changed the cut-off from 70 to greater or equal to 65. As a
    result, I was including more cases in my selection. I see that more
    observations were included as indicated by the increase in the
    number of rows (438 rows vs. 474 rows).
...
```{r}
d %>%
  select(student_id, percentage_earned) %>%
...
```{r}
ggplot(d, aes(x = FinalGradeCEMS)) +
  geom_histogram(fill = "violetred3")
```
...
```{r}
ggplot(d, aes(x = percentage_earned)) +
...
```{r}
ggplot(d, aes(x = Points_Possible, y = percentage_earned)) +
  geom_point(color= "steelblue3", shape ="square")
```
...
```{r}
m2 <- lm(FinalGradeCEMS ~ TimeSpent + subject + int, data = d)
summary(m2)
```
